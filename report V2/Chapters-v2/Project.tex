...
The proposed model \ref{p_model} has been built considering (with the help of) Rule 2. Since for a product we can not allocate more than 4 facings (which gives us a constant domain for facings), defining a seperate decision variable for facings is unnecessary. Rather we define a new decision variable instead of the previous ones, \\ \\
...
\]
\\
where $j \in \{1,2,3,4\}$. With this new approach, we have achieved to develop a linear model which can be reached in \ref{p_model}.